...
namespace logic
{
	...
	void CActor::Update(float dt)
	{
		...
		m_energy += 0.05f;			//aktualizujemy energiê...(zmniejszamy)
		...
		RotateHead(0);			//rotacja...

		//sprawdzamy œmieræ aktora...
		if (m_energy < 0)//zabrak³o energii...robot przestaje istnieæ...
			Kill();
	}
	...
	{
		//jeœli energia bêdzie mniejsza od 50 podmieniamy zestaw animacji
		if(m_energy < 50)	//mamy ma³o energii - obiekt podmienia animacje
			if(p_anim_set)
				SetAnimation(p_anim_set->GetMoveAnimBody(), p_anim_set->GetDefaultAnimHead());
	}
}//namespace logic
